...
export function getRightCollisions(
  layout: Layout,
  layoutItem: LayoutItem,
): TreeNode {
    // Original filtering
    const itemsToRight = layout.filter(
      l =>
        l.i !== layoutItem.i &&
        l.x === layoutItem.x + layoutItem.w &&
        l.y < layoutItem.y + layoutItem.h &&
        l.y + l.h > layoutItem.y
    );

    // Create the node for the current layoutItem
    const node: TreeNode = {
      value: layoutItem,
      children: [],
    };

    // For each item to the right, get all items to the right of it
    itemsToRight.forEach(rightItem => {
      // Exclude the current item from the layout for the recursive call
      const newLayout = layout.filter(item => item.i !== layoutItem.i);

      // Recursive call to get the right collisions for the rightItem
      const childNode = getRightCollisions(newLayout, rightItem);
...
export function getRightCollisionsLength(node: {value: LayoutItem, children: Array<{value: LayoutItem, children: Array<any>}>}): number {
  // Count the current node
  let count = 1;
  
  // Count all children nodes, if they exist
  if (Array.isArray(node.children)) {
    for (const child of node.children) {
      count += getRightCollisionsLength(child);
    }
  }
...
export function getRightResizables(root: TreeNode): TreeNode[] {
